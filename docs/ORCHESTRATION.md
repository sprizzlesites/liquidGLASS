...
- [orchestrator/fable] AUDIT WAVE COMPLETE. E1 (code): CONCERNS→fixed; E3 (requirements): PARTIAL→gaps fixed or logged; E4 (media): one HIGH corruption→fixed; E2 (interface flow) TERMINATED EARLY by session limit after confirming mobile dock views — MUST BE RERUN (§3.E2 prompt).
- [orchestrator/fable] FIX WAVE applied & verified: (1) E4-F1 file/folder-open + both drag-drop branches now route arrayBuffer→fileSetBytes — zero readAsText remain; re-verified with E4's own harnesses (29/29, 3/3, and 8b/8c end-to-end byte-identical; note e4-audit2 [8d] inlines a copy of the OLD code by design and fails forever — stale test, not a bug). (2) E1-#1 replaceSelectionOrFile b64 guard. (3) E3-F1 vmterm handles linux9p-packed with loud diagnostic + demo-mode notice names the build-vm-image Action. (4) E3-F2 runCode routes C/C++/asm to the VM tab with build hint. (5) E3-F3 64-bit constraint surfaced in boot banner. (6) E3-F4 Terminal-vs-VM tab tooltips. (7) E1-#2 termWrite→tw. (8) E1-#4 S.ctx persisted+hydrated (chips restored). (9) E1-#5 restoreSystemDefaults clears gitChanges/problems. All feature suites + spot regression suites green after fixes.
- [orchestrator/fable] REMAINING (priority order for whoever resumes): (1) RERUN AUDIT E2 (interface flow, desktop+mobile Playwright). (2) FIRST CI RUN of build-vm-image.yml on GitHub (needs Actions enabled on fork/repo; then linux9p mode goes live; verify caps.ld64 lands in manifest). (3) Requirement #2 depth: offline apk package dir in image + a discoverable "Add package" UI (E3-F5). (4) Optional: asm64 EXECUTION cloud workflow (mirror of VST3 pipeline) — currently documented-only. (5) Consider Pages deployment note for the branch (user must point Pages at fullterminal or use raw preview).
- [audit-E2/orchestrator] RERUN COMPLETE (run directly after 2 sub-agent session-limit kills; harness scratchpad/e2-audit.mjs): 22/22 PASS desktop+mobile — VM boots via real UI with serial echo, menus/onclicks all resolve, VST modal honest, binary tabs readonly+intact, mobile dock+VM reachable, no unexpected console errors. Fixed en route: Problems/Output panels were unshowable since main (.hidden !important vs inline display) — switchPanelTab now toggles the class. Report: docs/audit-E2.md. ALL FOUR AUDITS NOW COMPLETE.
- [orchestrator/opus] CI BUILD FIX (build-vm-image failed at [4/7]): root cause was `tar --delete .dockerenv` in build.sh corrupting the docker-export archive (PAX/GNU extended headers mangled by GNU tar --delete → "tar: Skipping to next header / Exiting with failure status", exit 2, killing the unguarded kernel/initramfs extraction). Fix: removed the --delete entirely (stray /.dockerenv is harmless in-guest) and hardened member listing (single `tar -tf` into a var instead of piping into `head`, which SIGPIPE-killed tar under pipefail). Verified: bash -n clean; extraction logic re-proven byte-identical on a synthetic PAX tar with ./-prefixed boot/vmlinuz-* + boot/initramfs-* members; de-risked steps 5-7 by confirming copy/v86 tools/fs2json.py + tools/copy-to-sha256.py still exist upstream (HTTP 200) and their argparse CLIs match build.sh's invocations. Docker build + Dockerfile were never the problem (image built fine). RE-RUN the workflow.
- [orchestrator/opus] CI BUILD FIX #2 (smoke test failed after the tar fix): the image now BUILDS + BOOTS to a root shell in CI (confirmed from the log: "Welcome to Alpine Linux 3.19, Kernel 6.6.142-0-virt on an i686", autologin, prompt). Two bugs in tests/vm/boot-linux-smoke.mjs: (1) prompt regex waited for `localhost:~#` but the getty wins the race vs OpenRC's hostname service so the prompt is `(none):~#` → 180s timeout → exit 1. Fixed: hostname-agnostic end-anchored regex `:~[#$]\s*$` (matches (none)/localhost/user@host, rejects mid-boot text + login: prompt — unit-tested). (2) latent: the serial TTY echoes the sent command line, so literal success tokens (echo A64=OK, the DONE marker, ...) would be seen in the echo before commands ran → premature/false eval. Fixed with the empty-string split trick (`A64''=OK` executes to `A64=OK` but the echoed text lacks the contiguous token); DONE marker split via splitTok(); RC=42/gcc/nasm were already echo-safe. Verified: node --check clean; echo-safety unit test (7/7: every searched token absent from echoed command, present in executed output); prompt-regex unit test (7/7). The `sysctl: unknown key` lines the user saw are harmless (OpenRC IPv6 tunables absent in virt kernel; prints [ok] and continues) — NOT the failure. RE-RUN the workflow; expect it to reach the compile/run session, pass hard reqs (gcc/nasm/RC=42/A64=OK), write manifest.json, and publish.
- [orchestrator/opus] CI TRIGGERING NOTE: "Retry" on a failed run replays the ORIGINAL commit SHA (never picks up new patches). There is no manual "Run workflow" button because workflow_dispatch only surfaces in the UI when the workflow exists on the DEFAULT branch (main) — ours is only on fullterminal. And the earlier smoke-test fixes touched tests/vm/ + docs/, outside the push-paths filter, so no auto-run fired. FIX: broadened build-vm-image.yml push paths to include tools/skel/** and tests/vm/boot-linux-smoke.mjs; pushing this (a workflow-file change, already in-paths) auto-starts a fresh run at HEAD. To get a durable manual "Run workflow" button, the workflow must also exist on main (needs user OK to touch main) OR use `gh workflow run build-vm-image.yml --ref fullterminal`.
